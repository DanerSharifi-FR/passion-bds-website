...
<main class="max-w-3xl mx-auto px-4 py-6 space-y-4">
    <div>
        <h1 class="text-xl font-semibold mb-1">Créer un nouvel allo</h1>
        <p class="text-sm text-slate-600">
            Définis le titre, le coût en points, la fenêtre de disponibilité et les admins responsables.
        </p>
    </div>
    ...
    <form method="POST" action="{{ route('admin.allos.store') }}" class="bg-white rounded-xl shadow-sm border border-slate-200 p-5 space-y-5">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                >
                <p class="mt-1 text-xs text-slate-500">
                    Laisse vide pour générer automatiquement à partir du titre.
                </p>
            </div>
        </div>
        ...
        <div>
            ...
            >
                @php
                    ...
                    $oldAdminIds = collect(old('admin_ids', []))->map(fn($v) => (int) $v)->all();
                @endphp

                @foreach($adminCandidates as $candidate)
                    <option value="{{ $candidate->id }}"
                            @if(in_array($candidate->id, $oldAdminIds, true)) selected @endif>
                        {{ $candidate->display_name ?? $candidate->university_email }}
                    </option>
                @endforeach
            </select>
            <p class="mt-1 text-xs text-slate-500">
                Les admins sélectionnés recevront les demandes pour cet allo.
            </p>
        </div>
