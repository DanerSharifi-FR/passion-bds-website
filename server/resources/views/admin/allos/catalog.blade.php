...
@section('title', "Catalogue Allos - P'AS'SION BDS")

@section('top_bar_buttons')
    <div class="flex bg-slate-900 p-1 rounded-lg border border-slate-700 ml-4">
        <a href="{{ route('admin.allos.requests') }}" id="tabRequests" class="px-4 py-1.5 rounded-md text-sm font-medium text-slate-400 hover:text-white transition-all">
        ...
        <a href="{{ route('admin.allos.catalog') }}" id="tabCatalog" class="px-4 py-1.5 rounded-md text-sm font-medium transition-all bg-indigo-600 text-white shadow">
    ...
    <script>
        ...
        async function loadPendingCount() {
            try {
                ...
                const pendingCount = (payload.data || []).length;
        ...
        function closeAlloModal() {
            document.getElementById('alloModal').classList.add('hidden');
