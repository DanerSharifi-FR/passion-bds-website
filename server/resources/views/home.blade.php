...
@section('content')
    ...
    <div id="machine-card"
        ...
        <div class="flex justify-between items-center mb-6 border-b-2 border-passion-pink-200 pb-4">
            <div class="flex items-center gap-3">
                ...
                <span
                    class="font-mono font-bold text-passion-red uppercase tracking-widest text-sm md:hidden">RDY</span>
            </div>
            <div
                class="status-indicator px-4 py-1 skew-box font-display font-black uppercase text-sm tracking-wider shadow-[4px_4px_0px_rgba(0,0,0,0.1)]">
        ...
        <div
            class="absolute -top-3 -right-3 w-8 h-8 bg-passion-fire-yellow border-2 border-passion-red z-10 skew-box"></div>
        <div
    ...
    <section id="arcade-zone"
             class="state-guest fan-zone-container w-full md:w-11/12 max-w-5xl md:transform md:-skew-x-6 p-1 relative z-10 mb-12 transition-transform duration-300 mx-auto">
        ...
        <div class="bg-[#9B1237] border-4 border-white py-6 px-4 md:p-10 md:transform md:skew-x-6">
            ...
            <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 border-b-2 border-white/20 pb-4 gap-4">
                ...
                @auth
                    @php($me = $leaderboardWidget['me'] ?? null)
                    @if($me)
                        <div class="flex flex-wrap gap-3 items-center w-full lg:w-auto">
                            <div class="bg-black/50 px-4 py-2 rounded border border-passion-fire-yellow flex items-center gap-2 shadow-[2px_2px_0px_#FFC94A] flex-shrink-0 whitespace-nowrap">
                                ...
                                <span class="font-bold text-white text-base md:text-lg">#{{ $me['rank'] }}</span>
                            </div>
                            <div class="bg-black/50 px-4 py-2 rounded border border-passion-fire-yellow flex items-center gap-2 shadow-[2px_2px_0px_#FFC94A] flex-shrink-0 whitespace-nowrap">
                                <span class="text-xs md:text-base">💰 Ton Solde:</span>
                                <span class="font-bold text-passion-fire-orange text-base md:text-lg">{{ $me['points'] }} pts</span>
                            </div>
                        </div>
                    @endif
                @endauth
            ...
            @php($podium = $leaderboardWidget['podium'] ?? [])
            @php($first = $podium[0] ?? null)
            @php($second = $podium[1] ?? null)
            @php($third = $podium[2] ?? null)

            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="bg-black/20 p-6 rounded border border-white/10 flex flex-col justify-between">
                    <div class="flex justify-between items-end mb-2">
                        <h3 class="font-display font-bold text-lg md:text-xl uppercase flex items-center gap-2">🏆 Top Joueurs</h3>
                    </div>
                    ...
                    @auth
                        ...
                        @if($me)
                            <div class="mt-auto">
                                @if(!empty($me['above_name']) && ($me['gap_to_above'] ?? 0) > 0)
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div class="flex-grow min-w-0">
                                            <div class="flex justify-between text-sm font-black uppercase text-white mb-1">
                                                <span class="truncate mr-2">#{{ $me['rank'] }} TOI</span>
                                                <span class="text-passion-fire-yellow flex-shrink-0">{{ $me['points'] }} pts</span>
                                            </div>
                                            <div class="w-full bg-black/40 h-3 rounded-full overflow-hidden border border-white/10">
                                                <div class="h-full bg-passion-fire-yellow progress-bar-striped"
                                                     style="width: {{ (int)($me['progress_to_above_pct'] ?? 0) }}%"></div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                ...
                <div id="game-card"
                     class="game-mode-action relative bg-passion-pink-500/20 p-6 rounded border-2 border-dashed border-passion-fire-yellow flex flex-col items-center justify-center text-center transition-colors hover:bg-passion-pink-500/30 min-h-[300px]">
                    ...
                    <h3 class="font-display font-black text-xl md:text-2xl uppercase tracking-tighter text-passion-fire-yellow drop-shadow-md break-words">
                        @auth
                            <span class="text-white">
                            {{-- get only name form display_name "name lastname" --}}
                                {{ explode(' ', auth()->user()->display_name)[0] }}
                        </span>
                            X
                        @endauth
                        <span class="text-white">APRÈM BDS</span>
                    </h3>
                    ...
                    <div class="show-if-action w-full flex flex-col items-center">
                        ...
                        <div class="text-left mb-4 p-4 text-sm bg-passion-pink-100/10 rounded lg:w-3/4 mx-auto">
                            <table>
                                <tr>
                        ...
                        @guest
                            ...
                            <a href="{{ route('login') }}"
                               class="show-if-guest bg-transparent border-2 border-white text-white font-black uppercase px-4 py-3 md:px-6 md:py-3 rounded text-xs md:text-sm hover:bg-white hover:text-[#9B1237] transition-all whitespace-nowrap">
                                Je me connecte
                            </a>
                        @endguest

                        {{-- <!-- Logged In Button -->
                        <button id="btn-action-upload" onclick="triggerFileSelection()"
                                class="show-if-logged-in bg-passion-fire-yellow text-[#9B1237] font-black uppercase px-6 py-3 rounded shadow-[4px_4px_0px_#000] hover:translate-y-1 hover:shadow-[2px_2px_0px_#000] transition-all flex flex-col items-center leading-none gap-1">
                            <span>J'ai la photo !</span>
                            <span class="text-[10px] opacity-80 font-mono">+100 pts à gagner</span>
                        </button>
                        <!-- Hidden File Input -->
                        <input type="file" id="proof-file" class="hidden" accept="image/*,video/*"
